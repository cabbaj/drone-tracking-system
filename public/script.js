...
socket.on("locationUpdate", (data) => {
  ...
  const distanceToRestrictedArea = map.distance(
    [latitude, longitude],
    restrictedAreaCenter,
  );
...
function addLogEntry(
  ...
  logEntry.innerHTML = `<strong>Drone ID:</strong> ${drone_id} | <strong>Date/Time:</strong> ${new Date().toLocaleString()} | <strong>Lat:</strong> ${latitude} | <strong>Lng:</strong> ${longitude} | <strong>Alt:</strong> ${altitude} | <strong>Speed:</strong> ${speed}`;
  if (isInRestrictedArea) {
    logEntry.innerHTML +=
      " <strong style='color: red;'>(อยู่ในพื้นที่ห้ามเข้า)</strong>";
  }
...
document.getElementById("confirmDrone").addEventListener("click", () => {
  ...
  const selectedOption = droneSelect.querySelector(
    `option[value="${selectedDroneId}"]`,
  );
  if (selectedOption) {
...
document
  ...
  .addEventListener("change", applyAltitudeFilter);
...
document.getElementById("toggleLog").addEventListener("click", () => {
  ...
  if (
    ...
    getComputedStyle(logContainer).display === "none"
  ) {
...
document
  ...
  .addEventListener("change", updateLogDisplay);
...
function updateLogDisplay() {
  ...
  const logs =
    filter === "all"
      ? logData
      : logData.filter((log) => log.drone_id === filter);

  [...logs].reverse().forEach((log) => {
    const logEntry = document.createElement("div");
    logEntry.innerHTML = `<strong>Drone ID:</strong> ${log.drone_id} | <strong>Date/Time:</strong> ${log.timestamp} | <strong>Lat:</strong> ${log.latitude} | <strong>Lng:</strong> ${log.longitude} | <strong>Alt:</strong> ${log.altitude} | <strong>Speed:</strong> ${log.speed}`;
...
function updateLogFilterOptions() {
  ...
  logFilter.innerHTML =
    '<option value="">ไม่แสดง</option><option value="all">แสดงทั้งหมด</option>';
