...
// Generate self-signed certificate with 2048-bit key for Node.js v25+ compatibility
...
app.get("/api/drones", (req, res) => {
  pool.query("SELECT drone_id FROM devices", (err, result) => {
    ...
    const availableDrones = result.rows.filter(
      (drone) => !selectedDrones.has(drone.drone_id),
    );
...
io.on("connection", (socket) => {
  ...
  socket.on("locationUpdate", (data) => {
    ...
    const query =
      "INSERT INTO location_logs(drone_id, latitude, longitude, altitude, speed) VALUES($1, $2, $3, $4, $5)";
    const values = [
      data.drone_id,
      data.latitude,
      data.longitude,
